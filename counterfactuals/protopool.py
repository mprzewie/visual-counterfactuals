...
class PrototypeChooser(nn.Module):
    ...
    def forward(self, x: torch.Tensor, gumbel_scale: int = 0) -> \
        ...
        ### addtional output - prototype activation
        x_det = self.distance_2_similarity(min_distances) # [b, p]
        x_avg_det = self.distance_2_similarity(avg_dist)
        x_det = torch.sqrt(x_det - x_avg_det) - 1
        ####
        ...
        if self.use_last_layer:
            ...
            x = x.sum(dim=-1)
        return x, min_distances, proto_presence, x_det.flatten(start_dim=1)  # [b,c,n]; [b, p]; [c, p, n]; [b, p]
    ...
    def post_backbone_classification(self):
        class _head(nn.Module):
            def forward(slf, bkb_features, gumbel_scale=10e3):
                bs = len(bkb_features)
                conv_features = self.add_on_layers(bkb_features)
                distances = self._l2_convolution(conv_features)  # [b, p, h, w]
                proto_presence = gumbel_softmax(self.proto_presence * gumbel_scale, dim=1, tau=0.5)
                min_distances = -F.max_pool2d(-distances,
                                              kernel_size=(distances.size()[2],
                                                           distances.size()[3])).reshape(bs, self.num_prototypes) #.squeeze()  # [b, p]
